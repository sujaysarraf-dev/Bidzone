...
class Store {
  ...
  login(email: string, role: UserRole = UserRole.BUYER) {
    const user: UserProfile = {
      uid: Math.random().toString(36).substr(2, 9),
      displayName: role === UserRole.INSTITUTION ? "NICA Asia Bank" : "Demo User",
      email,
      role,
      status: UserStatus.ACTIVE,
      createdAt: new Date().toISOString(),
      institutionName: role === UserRole.INSTITUTION ? "NICA Asia Bank" : undefined,
    };
    this.user = user;
    this.saveUser();
    return user;
